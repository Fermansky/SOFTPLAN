...
历史上曾用 `document_parsing_tasks` 表达版面分析阶段任务，当前 schema 已固定拆分为：

- `layout_analysis_tasks`：版面分析阶段任务
- `document_parsing_tasks`：文档解析聚合父任务

仓库启动时不再负责把旧库自动迁移到这一结构；当前只保证全新数据库按现有 schema 初始化。

### 9.2 旧图片语义字段保留

`extracted_images.semantic_description*` 仍然保留，但现在属于当前 schema 的兼容读取字段，不再依赖启动时迁移或回填成为主复用依据。
