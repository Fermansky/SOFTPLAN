...
- 统一 `backend`、`llm-service`、`file-convert-service` 三个 Python 服务的日志配置方式。
...
- 轻量依赖：
  ...
  - `asgi-correlation-id`
- 当前仓库实现：
  - 共享实现位于 `common/softplan_common/logging.py`
  - 每个服务的 `app/core/logging.py` 仅保留薄封装或 re-export，避免复制整份实现
  - 共享模块暴露：
...
- 本地开发约定：
  - 在仓库根目录执行 `pip install -e .`，将 `softplan_common` 安装到当前虚拟环境
  - Docker 镜像构建时显式复制并安装该公共包，不依赖 `PYTHONPATH` 注入
...
- `backend -> llm-service` 必须透传 `X-Request-ID`
- `backend -> file-convert-service` 必须透传 `X-Request-ID`
- worker 场景下若无上游请求上下文，可退化为使用任务 ID 作为链路标识

### 旧头兼容

- `file-convert-service` 历史上使用 `X-Convert-Task-Id`
- 当前兼容策略：
  - 若请求缺少 `X-Request-ID`，则回退读取 `X-Convert-Task-Id`
  - 日志统一写入 `request_id`
...
- 所有服务启动时最早调用 `configure_logging(service_name)`。
- 所有服务在 FastAPI 应用创建后立即安装 request id 中间件。
...
- `llm-service/app/api/routers/llm.py`
- `llm-service/app/services/llm_client.py`
- `file-convert-service/app/main.py`
- `file-convert-service/app/api/routers/converters.py`
