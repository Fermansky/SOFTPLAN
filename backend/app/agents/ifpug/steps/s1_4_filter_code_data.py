"""子任务 1.4：识别"代码数据 / 参考数据"并打 Exclusion 标签。

形态与 s1_2 完全一致（分类型 / 打包调用 / 仅追加 Exclusion），区别仅在
prompt 判断口径与标签 tag。详见 prompt 文件 ``ifpug_s1_4_filter_code_data.txt``。
"""
...
from ..domain import EXCLUDED_BY_CODE_DATA, DataEntity, Exclusion

logger = logging.getLogger(__name__)

_CALLER_SERVICE_NAME = "backend.agent.ifpug.s1_4_filter_code_data"
_DEFAULT_PROMPT_PATH = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "prompts"
    / "ifpug_s1_4_filter_code_data.txt"
)
_PROMPT_ENV_VAR = "IFPUG_S1_4_FILTER_CODE_DATA_PROMPT_PATH"
_DEFAULT_TEMPERATURE = 0.1
_STEP_NAME = "ifpug.s1_4_filter_code_data"
_TAG = EXCLUDED_BY_CODE_DATA
_MAX_ID_LENGTH = 64
_MAX_RATIONALE_LENGTH = 600
_MAX_CONTEXT_FIELD_LENGTH = 8000


class FilterCodeDataPromptError(RuntimeError):
    """读取 system prompt 文件失败时抛出。"""


class FilterCodeDataAgentError(RuntimeError):
    """子任务 1.4 无法完成时抛出。"""
...
_prompt_loader = PromptLoader(
    ...
    error_cls=FilterCodeDataPromptError,
    label="ifpug s1_4",
)
...
load_filter_code_data_prompt = _prompt_loader.cached_loader


def get_filter_code_data_prompt_snapshot() -> tuple[str, str | None]:
...
@dataclass(frozen=True)
class FilterCodeDataAgentResult:
...
def _normalize_optional_text(value: str | None, *, name: str, max_length: int) -> str:
    ...
    if len(normalized) > max_length:
        raise FilterCodeDataAgentError(
...
def _ensure_string(payload: Any, *, field_name: str, max_length: int) -> str:
    if not isinstance(payload, str):
        raise FilterCodeDataAgentError(
            f"filter_code_data agent returned invalid field: {field_name}"
        )
    normalized = payload.strip()
    if not normalized:
        raise FilterCodeDataAgentError(
            f"filter_code_data agent returned empty field: {field_name}"
        )
    if len(normalized) > max_length:
        raise FilterCodeDataAgentError(
            f"filter_code_data agent returned field exceeding limit: {field_name}>{max_length}"
        )
    return normalized
...
def _parse_verdicts_payload(payload: dict[str, Any]) -> list[ExclusionVerdict]:
    ...
    if not isinstance(excluded_raw, list):
        raise FilterCodeDataAgentError(
            "filter_code_data agent returned invalid field: excluded"
        )
    ...
    for index, raw in enumerate(excluded_raw):
        if not isinstance(raw, dict):
            raise FilterCodeDataAgentError(
                f"filter_code_data agent returned invalid item at excluded[{index}]"
            )
...
def build_filter_code_data_user_prompt(
    *,
...
def run_filter_code_data_agent(
    *,
    entities: list[DataEntity],
    counting_scope: str = "",
    user_requirements: str = "",
    ...
    if not entities:
        raise FilterCodeDataAgentError(
            "filter_code_data agent requires at least one candidate entity"
        )
    ...
    system_prompt = load_filter_code_data_prompt()
    prompt_path, prompt_hash = get_filter_code_data_prompt_snapshot()
    client = get_llm_service_client(config_id=config_id, session=session)

    try:
        result, error = client.chat(
            prompt=build_filter_code_data_user_prompt(
                entities=entities,
    ...
    except (LlmChatPersistenceError, LlmConfigError, FilterCodeDataPromptError):
        raise

    if error is not None or result is None:
        raise FilterCodeDataAgentError(error or "filter_code_data agent failed")

    try:
        parsed = parse_object(result.text)
    except LlmJsonParseError as exc:
        raise FilterCodeDataAgentError(
            f"filter_code_data agent returned invalid json: {exc}"
        ) from exc

    verdicts = _parse_verdicts_payload(parsed)

    return FilterCodeDataAgentResult(
        verdicts=verdicts,
        model=result.model,
...
class FilterCodeDataStep:
    """子任务 1.4 的 PipelineStep 薄包装。"""

    name = _STEP_NAME

    def run(self, ctx: IfpugContext) -> StepRecord:
        if ctx.session is None:
            raise FilterCodeDataAgentError("ifpug pipeline requires ctx.session")

        active = ctx.active_entities()
        active_count = len(active)

        if not active:
            logger.info("ifpug s1_4 skipped: no active candidate entities")
            return StepRecord(
                name=self.name,
        ...
        result = run_filter_code_data_agent(
            entities=active,
        ...
        if outcome.unknown_ids or outcome.duplicate_ids or outcome.already_excluded_ids:
            ...
            logger.warning(
                "ifpug s1_4 received anomalous verdicts: unknown=%s duplicates=%s already_excluded=%s",
                outcome.unknown_ids,
