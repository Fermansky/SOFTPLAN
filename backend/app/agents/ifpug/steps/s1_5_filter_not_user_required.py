"""子任务 1.5：识别"非用户业务需求驱动"的实体并打 Exclusion 标签。

形态与 s1_2 / s1_4 完全一致（分类型 / 打包调用 / 仅追加 Exclusion），区别
仅在 prompt 判断口径与标签 tag。详见 prompt 文件
``ifpug_s1_5_filter_not_user_required.txt``。
"""
...
from ..domain import EXCLUDED_BY_NOT_USER_REQUIRED, DataEntity, Exclusion

logger = logging.getLogger(__name__)

_CALLER_SERVICE_NAME = "backend.agent.ifpug.s1_5_filter_not_user_required"
_DEFAULT_PROMPT_PATH = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "prompts"
    / "ifpug_s1_5_filter_not_user_required.txt"
)
_PROMPT_ENV_VAR = "IFPUG_S1_5_FILTER_NOT_USER_REQUIRED_PROMPT_PATH"
_DEFAULT_TEMPERATURE = 0.1
_STEP_NAME = "ifpug.s1_5_filter_not_user_required"
_TAG = EXCLUDED_BY_NOT_USER_REQUIRED
_MAX_ID_LENGTH = 64
_MAX_RATIONALE_LENGTH = 600
_MAX_CONTEXT_FIELD_LENGTH = 8000


class FilterNotUserRequiredPromptError(RuntimeError):
    """读取 system prompt 文件失败时抛出。"""


class FilterNotUserRequiredAgentError(RuntimeError):
    """子任务 1.5 无法完成时抛出。"""
...
_prompt_loader = PromptLoader(
    ...
    error_cls=FilterNotUserRequiredPromptError,
    label="ifpug s1_5",
)
...
load_filter_not_user_required_prompt = _prompt_loader.cached_loader


def get_filter_not_user_required_prompt_snapshot() -> tuple[str, str | None]:
...
@dataclass(frozen=True)
class FilterNotUserRequiredAgentResult:
...
def _normalize_optional_text(value: str | None, *, name: str, max_length: int) -> str:
    ...
    if len(normalized) > max_length:
        raise FilterNotUserRequiredAgentError(
...
def _ensure_string(payload: Any, *, field_name: str, max_length: int) -> str:
    if not isinstance(payload, str):
        raise FilterNotUserRequiredAgentError(
            f"filter_not_user_required agent returned invalid field: {field_name}"
        )
    normalized = payload.strip()
    if not normalized:
        raise FilterNotUserRequiredAgentError(
            f"filter_not_user_required agent returned empty field: {field_name}"
        )
    if len(normalized) > max_length:
        raise FilterNotUserRequiredAgentError(
            f"filter_not_user_required agent returned field exceeding limit: {field_name}>{max_length}"
        )
    return normalized
...
def _parse_verdicts_payload(payload: dict[str, Any]) -> list[ExclusionVerdict]:
    ...
    if not isinstance(excluded_raw, list):
        raise FilterNotUserRequiredAgentError(
            "filter_not_user_required agent returned invalid field: excluded"
        )
    ...
    for index, raw in enumerate(excluded_raw):
        if not isinstance(raw, dict):
            raise FilterNotUserRequiredAgentError(
                f"filter_not_user_required agent returned invalid item at excluded[{index}]"
            )
...
def build_filter_not_user_required_user_prompt(
    *,
...
def run_filter_not_user_required_agent(
    *,
    entities: list[DataEntity],
    counting_scope: str = "",
    user_requirements: str = "",
    ...
    if not entities:
        raise FilterNotUserRequiredAgentError(
            "filter_not_user_required agent requires at least one candidate entity"
        )
    ...
    system_prompt = load_filter_not_user_required_prompt()
    prompt_path, prompt_hash = get_filter_not_user_required_prompt_snapshot()
    client = get_llm_service_client(config_id=config_id, session=session)

    try:
        result, error = client.chat(
            prompt=build_filter_not_user_required_user_prompt(
                entities=entities,
    ...
    except (LlmChatPersistenceError, LlmConfigError, FilterNotUserRequiredPromptError):
        raise

    if error is not None or result is None:
        raise FilterNotUserRequiredAgentError(
            error or "filter_not_user_required agent failed"
        )
    ...
    try:
        ...
        raise FilterNotUserRequiredAgentError(
            f"filter_not_user_required agent returned invalid json: {exc}"
        ) from exc

    verdicts = _parse_verdicts_payload(parsed)

    return FilterNotUserRequiredAgentResult(
        verdicts=verdicts,
        model=result.model,
...
class FilterNotUserRequiredStep:
    """子任务 1.5 的 PipelineStep 薄包装。"""

    name = _STEP_NAME

    def run(self, ctx: IfpugContext) -> StepRecord:
        if ctx.session is None:
            raise FilterNotUserRequiredAgentError("ifpug pipeline requires ctx.session")

        active = ctx.active_entities()
        active_count = len(active)

        if not active:
            logger.info("ifpug s1_5 skipped: no active candidate entities")
            return StepRecord(
                name=self.name,
        ...
        result = run_filter_not_user_required_agent(
            entities=active,
        ...
        if outcome.unknown_ids or outcome.duplicate_ids or outcome.already_excluded_ids:
            ...
            logger.warning(
                "ifpug s1_5 received anomalous verdicts: unknown=%s duplicates=%s already_excluded=%s",
                outcome.unknown_ids,
