...
class ConvertPdfToMarkdownFileRead(BaseModel):
    filename: str | None = None
    markdown: str
    image_hashes: dict[str, str] = Field(default_factory=dict)
    images: list[InlineImageRead] = Field(default_factory=list)
...
def _normalize_pdf_filename(filename: str | None) -> str | None:
    if filename is None:
        return None
    normalized_filename = filename.strip()
    return normalized_filename or None


def _validate_pdf_filename(filename: str | None) -> str | None:
    normalized_filename = _normalize_pdf_filename(filename)
...
@router.post("/pdf-to-markdown/file", response_model=ConvertPdfToMarkdownFileRead)
async def convert_pdf_to_markdown_from_file(
    request: Request,
    file: UploadFile = File(...),
    model: str | None = Form(default=None),
    converter: MarkerPdfToMarkdownConverter = Depends(get_marker_pdf_to_markdown_converter),
) -> ConvertPdfToMarkdownFileRead:
    request_id = get_request_id()
    trace_task_id = request.headers.get(LEGACY_REQUEST_ID_HEADER) or request_id
    ...
    target_model = _resolve_pdf_model(model)

    logger.info(
        "Start PDF to markdown conversion from uploaded file",
        extra=build_log_extra(
            "pdf_to_markdown.file.started",
            request_id=request_id,
            task_id=trace_task_id,
            filename=filename,
            model=target_model,
    ...
    try:
        render_result = converter.render(payload)
    except RuntimeError as exc:
        logger.exception(
            "Marker converter is unavailable for uploaded file",
            extra=build_log_extra(
                "pdf_to_markdown.file.converter_unavailable",
                request_id=request_id,
                task_id=trace_task_id,
                filename=filename,
                model=target_model,
        ...
        logger.exception(
            "Failed to convert uploaded PDF to markdown",
            extra=build_log_extra(
                "pdf_to_markdown.file.failed",
                request_id=request_id,
                task_id=trace_task_id,
                filename=filename,
                model=target_model,
    ...
    logger.info(
        "Finished PDF to markdown conversion from uploaded file",
        extra=build_log_extra(
            "pdf_to_markdown.file.succeeded",
            request_id=request_id,
            task_id=trace_task_id,
            filename=filename,
            model=target_model,
    ...
    return ConvertPdfToMarkdownFileRead(
        filename=filename,
        ...
        images=[
            InlineImageRead(
                ...
                width=item.width,
                height=item.height,
                content_base64=b64encode(item.payload).decode("ascii"),
            )
            for item in render_result.images
        ],
    )
