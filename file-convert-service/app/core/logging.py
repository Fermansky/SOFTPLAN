"""File convert service logging infrastructure."""
...
LEGACY_REQUEST_ID_HEADER = "X-Convert-Task-Id"
...
def _sanitize_log_extra_key(key: str, existing_keys: set[str]) -> str:
    if key not in _LOG_RECORD_RESERVED_FIELDS and key not in existing_keys:
        return key

    sanitized = _RESERVED_LOG_EXTRA_ALIASES.get(key, f"extra_{key}")
    if sanitized not in _LOG_RECORD_RESERVED_FIELDS and sanitized not in existing_keys:
        return sanitized

    suffix = 2
    while True:
        candidate = f"{sanitized}_{suffix}"
        if candidate not in _LOG_RECORD_RESERVED_FIELDS and candidate not in existing_keys:
            return candidate
        suffix += 1
...
def build_log_extra(event: str, **fields: Any) -> dict[str, Any]:
    ...
    for key, value in fields.items():
        if value is None:
            continue
        extra[_sanitize_log_extra_key(key, set(extra))] = value
    return extra
...
def install_request_id_middleware(app: FastAPI) -> None:
    ...
    async def request_id_middleware(request: Request, call_next):
        ...
        if request_id is None:
            legacy_request_id = request.headers.get(LEGACY_REQUEST_ID_HEADER)
